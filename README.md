...
～～the `publish-action` command add three arguments: `dir`, `tag-prefix` and `config`, you can use first two arguments publish once, and using third argument publish more crates, when `config`'s value is `""`, this command will found `.github/publish.yml` config file. ～～
